...
void decode(char *filename) {
   ...
   for (n=0 ; n<10 ; n++) {
	...
	g4->SetPoint(i, time[3][i], waveform[3][i]);
      ...
      TCanvas *c1 = new TCanvas("c1","sPHENIX Calorimeter Tile Test");	 
    
      c1-> Divide(1,4);
      c1->cd(2);
      
      g1 -> GetYaxis()->SetRangeUser(-0.01,0.04);
      g1 -> SetTitle("OH-2-1 Spectrum");
      g1 -> Draw("ACP");

      c1->cd(4);
      g2->GetYaxis()->SetRangeUser(-0.01,0.04);
      g2->SetTitle("IH-1-16 Spectrum");
      g2->Draw("ACP");

      c1->cd(3);
      g3->GetYaxis()->SetRangeUser(-0.01,0.04);
      g3->SetTitle("OH-1-15 Spectrum");
      g3->Draw("ACP");

      c1->cd(1);
      g4->GetYaxis()->SetRangeUser(-0.01,0.04);
      g4->SetTitle("IH-2-12 Spectrum");
      g4->Draw("ACP");
   ...
   TCanvas *c2 = new TCanvas("c1","sPHENIX Four Tile Test");
   c2->Divide(1,4);
